  ...
  describe('Deposit', function () {
    describe('Validations', function () {
      it('Should revert with the right error ', async function () {
        ...
        await expect(
          payment.deposit(expectedAmount, await MultiSigWallet.getAddress(), {
            value: depositAmount,
          }),
    ...
    describe('Payment', function () {
      it('Should deposit funds', async function () {
        const { payment, MultiSigWallet, owner } = await loadFixture(
          deployOneYearLockFixture,
        )
        const depositAmount = ethers.parseUnits('1', 'ether')
        const expectedAmount = ethers.parseUnits('1', 'ether')
        await payment.deposit(expectedAmount, await MultiSigWallet.getAddress(), {
          value: depositAmount,
        })
